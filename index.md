...
**"Users around the world want models that run on their own devices."** The piece cites its OpenClaw example as evidence of this and uses it to motivate the urgency of intervention. The actual usage data points elsewhere. <span class="cite">ChatGPT crossed 900M weekly active users by February 2026<span class="src"><strong>ChatGPT WAU</strong><br>OpenAI announced ChatGPT reached 900M weekly active users in Feb 2026, up from 800M in Oct 2025 and more than double the 400M of Feb 2025. Gemini passed 750M monthly users. Claude's web traffic share tripled in Q1 2026.<br><a href="https://nerdynav.com/chatgpt-statistics/" target="_blank" rel="noopener">ChatGPT usage stats</a></span></span>, Gemini passed 750M monthly users, and the consumer-cloud LLM market sits in the hundreds of millions of weekly actives. Local-runtime adoption is real and growing — <span class="cite">Ollama crossed 112M cumulative model pulls for Llama 3.1 alone, with 180% year-over-year growth<span class="src"><strong>Local LLM adoption</strong><br>Ollama is the most popular local LLM runtime; 112M cumulative model pulls for Llama 3.1, 180% YoY growth. Cumulative pulls are not weekly active users — local LLMs are a real and growing segment, but not yet the modal user experience.<br><a href="https://medium.com/@rosgluk/local-llm-hosting-complete-2025-guide-ollama-vllm-localai-jan-lm-studio-more-f98136ce7e4a" target="_blank" rel="noopener">Local LLM hosting guide (2025)</a></span></span> — but cumulative pulls are not weekly actives, and the gap between hundreds-of-millions-of-WAU consumer-cloud and a developer-and-enthusiast local segment is enormous. Citing a fast-growing local-agent framework is not the same as showing that on-device is what users are choosing in aggregate; the actual revealed preference, today, is overwhelmingly cloud.

The Stanford "Intelligence Per Watt" study the piece cites — local-model query accuracy rising from 23% to 71% between 2023 and 2025 — is real and accurately summarized, but it measures *capability*, not *adoption*. The capability has crossed a threshold. Whether users want that capability locally, in numbers comparable to the cloud market, is a different empirical question, and the answer right now is "not yet."
